...
document.addEventListener(
  ...
  (e) => {
    ...
    if (!Ø(".player").classList.contains("hidden")) {
      videoTimeAtTouch = Ø("video").currentTime;
      videoPausedAtTouch = Ø("video").paused;
    }
  ...
  (e) => {
    ...
    if (!gesture) {
      if (
        ...
        startTouchY <= playerSize.y + playerSize.height - 50
      ) {
        gesture = "seek";
...
document.addEventListener("touchend", async (e) => {
  ...
  if (gesture === "LTR" && diffX > swipeThreshold) {
    ...
    if (!videoPausedAtTouch) Ø("video").play();
    Ø(".osd").textContent = "";
  }
  gesture = "";
  isSomethingScrolling = false;
